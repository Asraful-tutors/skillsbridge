...
export async function startSkillAssessment(
  options?: StartSkillAssessmentOptions
) {
  ...
  const questions = path.skills.flatMap((s: any, i: any) =>
    s.questions.map(
      (q: any, j: any) =>
        ({
          skillId: q.skillId,
    ...
    case "fail-if-active": {
      ...
      if (existing)
        throw new PublicError(
          409,
          "You already have an active skill assessment"
        );
...
export async function answerSkillQuestion(
  ...
  const record = await prisma.skillAssessmentRecord.findUnique({
