...
CREATE TABLE `SkillAssessmentRecord` (
    ...
    INDEX `SkillAssessmentRecord_userId_sealed_idx`(`userId`, `sealed`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- CreateTable
CREATE TABLE `SkillQuestionRecord` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    ...
    `sealed` BOOLEAN NOT NULL DEFAULT false,
    `skillType` ENUM('HARD', 'SOFT') NOT NULL,
    `data` JSON NULL,

    UNIQUE INDEX `SkillQuestionRecord_recordId_index_key`(`recordId`, `index`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `SkillAssessmentRecord` ADD CONSTRAINT `SkillAssessmentRecord_userId_fkey` FOREIGN KEY (`userId`) REFERENCES `User`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `SkillQuestionRecord` ADD CONSTRAINT `SkillQuestionRecord_recordId_fkey` FOREIGN KEY (`recordId`) REFERENCES `SkillAssessmentRecord`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `SkillQuestionRecord` ADD CONSTRAINT `SkillQuestionRecord_questionId_fkey` FOREIGN KEY (`questionId`) REFERENCES `SkillQuestion`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `SkillQuestionRecord` ADD CONSTRAINT `SkillQuestionRecord_skillId_fkey` FOREIGN KEY (`skillId`) REFERENCES `Skill`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
